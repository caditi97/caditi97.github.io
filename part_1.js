...
(function(){
  ...
  function createSquares(){
    ...
    for (let i = 1; i <= DIM_X*2; i+=2){
      ...
      for (let j = 1; j <= DIM_Y*2; j+=2){
        let y = sqrHeight * j;
        squares[index] = {x:x, y:y, width:sqrWidth};
        index+=1;
      }
    }
  }
  ...
  // function inCircle(square){
  //   sqrCenter = {x:square.x + square.width/2, y:square.y + square.width/2};
  //   distFromCenter = (((sqrCenter.x - centerX)**2) + ((sqrCenter.y - centerY)**2))**(0.5)
  ...
  function isBlue(index){
    for (let angle = 0; angle < 360; angle+=10){
      let point = getPoint(angle * (Math.PI / 180));
      if (inSquare(squares[index],point)){
        return true;
      }
    }
    return false;
